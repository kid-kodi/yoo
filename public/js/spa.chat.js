...
spa.chat = (function () {
  ...
  // Throws    : none
  //
  setSliderPosition = function ( $tapped, position_type, callback ) {
    var
    ...
    $tapped.removeClass( chat_postion );
    $tapped.animate(
      { height : height_px },
      animate_time,
      function () {
        $tapped.addClass( position_type );
        if ( callback ) { callback( $tapped ); }
      }
    );
    return true;
  };
  ...
  scrollChat = function( $room ) {
    var $msg_log = $room.find( '.spa-chat-msg-log' );
    $msg_log.animate(
  ...
  writeChat = function ( $room, person, text, is_user ) {
    var msg_class = is_user
      ? 'spa-chat-msg-log-me' : 'spa-chat-msg-log-msg',
      $msg_log = $room.find( '.spa-chat-msg-log' );

      if( is_user ){
        $msg_log.append(
          '<div class="' + msg_class + '">' 
          + '<p>' + spa.util_b.encodeHtml(text) + '</p>' 
          + '</div>'
        );
      }
      else{
  ...
  onTapToggle = function ( event ) {
    var 
    position_type = 'closed',
    $tapped   = $( event.elem_target ).closest('.spa-chat');
    ...
    if ( position_type === 'opened' ) {
      setSliderPosition( $tapped, 'closed' );
    }
    else if ( position_type === 'closed' ){
      setSliderPosition( $tapped, 'opened' );
    }
    return false;
  };
  ...
  onTapRemove = function ( event ) {
    var
    $tapped = $( event.elem_target ).closest('.spa-chat');
    //setSliderPosition( $tapped, 'hidden' );
    removeSlider( $tapped );
    return false;
  };
  ...
  onSubmitMsg = function ( event ) {
    ...
    form_map = {},
    $form    = $(this),
    dest_id  = $form.find( 'input[type=hidden]' ).val(),
    msg_text = $form.find( 'input[type=text]'   ).val();
    ...
    configMap.chat_model.send_msg( form_map );
  ...
  onTapList = function ( event ) {
    ...
    $new_room = jqueryMap.$append_target
    .find('.spa-chat[data-id=' + arg_map.new_chatee.id + ']');

    if ( $new_room.length > 0 ) { return false; }
  ...
  onSetchatee = function ( event, arg_map ) {
    ...
    $old_room, $new_room, $head, $close, 
    $form, $send, $input, chat_html;

    chat_html = String()
        + '<div class="spa-chat" data-id="'+new_chatee.id+'">'
          + '<div class="spa-chat-head">'
            + '<div class="spa-chat-head-title">'
              + new_chatee.name
            + '</div>'
          + '<div class="spa-chat-closer ion-close"></div>'
          + '</div>'
          + '<div class="spa-chat-sizer">'
            + '<div class="spa-chat-msg">'
              + '<div class="spa-chat-msg-log"></div>'
              + '<div class="spa-chat-msg-in">'
                + '<form class="spa-chat-msg-form">'
                  ...
                  + '<input type="hidden" value="'+new_chatee.id+'"/>'
                  + '<input type="submit" style="display:none"/>'
                + '</form>'
              + '</div>'
            + '</div>'
    ...
    $old_room = jqueryMap.$append_target
    .find('.spa-chat[data-id=' + new_chatee.id + ']');

    if($old_room.length > 0){ 
      $input = $old_room.find( '.spa-chat-msg-in input[type=text]');
      $input.focus();
      return false;
    };

    jqueryMap.$append_target.append( chat_html );
    setJqueryMap();
    setPxSizes();
    ...
    $head  = $new_room.find( '.spa-chat-head' );
    $close = $new_room.find( '.spa-chat-closer' );
    $input = $new_room.find( '.spa-chat-msg-in input[type=text]');
    $form  = $new_room.find( 'form.spa-chat-msg-form' );

    $head.bind(     'utap', onTapToggle );
    $close.bind(    'utap', onTapRemove );

    $form.submit( onSubmitMsg );

    setSliderPosition( $new_room, 'opened' );
  ...
  onUpdatechat = function ( event, msg_map ) {
    var
      is_user, $room, room_id,
      sender_id = msg_map.sender_id,
      dest_id   = msg_map.dest_id,
      msg_text  = msg_map.msg_text,
      chatee    = configMap.people_model.get_by_cid( dest_id ),
    ...
    $room     = jqueryMap.$append_target
      .find('.spa-chat[data-id=' + room_id + ']');

    writeChat( $room, sender, msg_text, is_user );
  ...
  initModule = function ( $append_target ) {
    ...
    $list_box = jqueryMap.$append_target;
